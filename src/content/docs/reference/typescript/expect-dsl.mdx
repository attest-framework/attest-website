...
| ----------- | ----------------------- | ---------------- | ---------------------------------------------------------------------------------- |
| `threshold` | `number \| "dynamic"`   | `0.8`            | Minimum cosine similarity score (0.0 to 1.0), or `"dynamic"` for σ-based drift     |
...
// Dynamic threshold — engine compares against historical mean ± σ
attestExpect(result).outputSimilarTo("reference text", { threshold: "dynamic" });
```
...
| `threshold` | `number \| "dynamic"`   | `0.8`            | Minimum judge score (0.0 to 1.0), or `"dynamic"` for σ-based drift          |
...
// Dynamic threshold — catch regressions without picking an absolute number
