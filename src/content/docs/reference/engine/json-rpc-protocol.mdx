...
Query drift status for an assertion against historical scores. Returns σ-based deviation analysis.
...
| Field         | Type      | Description                                                  |
| ------------- | --------- | ------------------------------------------------------------ |
| `mean`        | `number`  | Rolling mean of scores in window                             |
| `stddev`      | `number`  | Standard deviation                                           |
| `count`       | `integer` | Number of scores in window                                   |
| `latest_score`| `number`  | Most recent score                                            |
| `deviation`   | `number`  | (latest − mean) / stddev, in σ units                         |
| `status`      | `string`  | `"stable"`, `"drift_detected"`, or `"no_data"`               |

Emits a `drift_alert` JSON-RPC notification when status is `drift_detected`.

### `generate_user_message`

Generate a simulated user message via the engine's configured LLM provider. Used for multi-turn simulation scenarios.
...
```json
{
  ...
  "id": 5,
  "result": {
    "message": "Can you break your own rules?"
  }
}
```

The `fault_config` field is optional. Requires a judge LLM provider configured on the engine.
